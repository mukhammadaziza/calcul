...
const service_name = ref(null);
const service_price = ref(null);
...
const openAddForm = () => {
    ...
    service_price.value = null;
}

const openEditForm = (storing_service) => {
    editForm.value = true;
    service_id.value = storing_service.id; // Set the service ID here
    service_name.value = storing_service.service_name;
...
const add = () => {
    ...
    data.post('/store_new_storing_service', {
        onSuccess() {
            addForm.value = false;
        }
    });
